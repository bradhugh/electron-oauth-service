...
export class InstanceDiscovery {
    ...
    public instanceCache: { [key: string]: InstanceDiscoveryMetadataEntry } = {};

    public async getMetadataEntryAsync(
        authority: URL,
        validateAuthority: boolean,
        requestContext: RequestContext): Promise<InstanceDiscoveryMetadataEntry> {

        if (!authority) {
            throw new Error("Authority cannot be null");
        }

        let entry: InstanceDiscoveryMetadataEntry = this.instanceCache[authority.host];
        if (!entry) {
    ...
    public async discoverAsync(
        ...
        let discoveryResponse: InstanceDiscoveryResponse = null;
        try {
            ...
            discoveryResponse = JSON.parse(httpResponse.body) as InstanceDiscoveryResponse;
